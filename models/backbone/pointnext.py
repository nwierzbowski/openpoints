...
from ..layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
    three_interpolation, get_aggregation_feautres, spatial_group, spatial_self_group
...
class LocalAggregation(nn.Module):
    ...
    def forward(self, pf) -> torch.Tensor:
        p, f = pf
        dp, fj = spatial_self_group(p, f, self.nsample)
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(channels) - 1):
            conv_kwargs = {'norm_args': norm_args if not is_head else None,
                          'act_args': None if i == len(channels) - 2
                                               and (self.use_res or is_head) else act_args}
            if conv_args is not None:
                conv_kwargs.update(conv_args)
            convs.append(create_conv(channels[i], channels[i + 1], **conv_kwargs))
        self.convs = nn.Sequential(*convs)
        if not is_head:
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            npoint = p.shape[1] // self.stride if not self.all_aggr else p.shape[1]
            ...
            fj = get_aggregation_feautres(new_p, dp, fi if fi is not None else new_p, fj, feature_type=self.feature_type)
            f = self.pool(self.convs(fj))
            if self.use_res:
...
class InvResMLP(nn.Module):
    def __init__(self,
        ...
        self.convs = LocalAggregation([in_channels, in_channels],
                                      ...
                                      nsample=nsample, conv_args=conv_args,
                                      **aggr_args, **kwargs)
        if num_posconvs < 1:
            channels = []
        elif num_posconvs == 1:
...
class PointNextEncoder(nn.Module):
    ...
    Args:
        ...
        nsample (intorList[int], optional): the number of neighbors to gather for each block. Defaults to 32.
    ...
    def _make_enc(self, block, channels, sas, blocks, stride, nsample, is_head=False):
        ...
        for i in range(1, blocks):
            layers.append(block(self.in_channels,
                                ...
                                norm_args=self.norm_args, act_args=self.act_args, nsample=nsample[i],
    ...
    def forward_cls_feat(self, p0, f0=None):
        ...
        # Global max pooling across points: (B, C, N) -> (B, C)
        return f0.max(dim=-1)[0]
